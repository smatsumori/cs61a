...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14A
            sub_expressions = clause.second

            # No else case
            if expressions == nil:
                return None
            else:
                if sub_expressions == nil:
                    return test
                else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        val = scheme_eval(item.second.first, env)
        vals = Pair(val, vals)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            op = scheme_optimized_eval(first, env)
            check_procedure(op)
            args = rest.map(lambda expr: scheme_optimized_eval(expr, env))
            result = scheme_apply(op, args, env)
            # END Extra Credit
    return result
