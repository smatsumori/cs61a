...
def scheme_read(src):
    ...
    if val == 'nil':
        # BEGIN PROBLEM 1
        # if the token is null return 'nil' object
        return nil
        # END PROBLEM 1
    elif val == '(':
        # BEGIN PROBLEM 1
        # start of a Pair
        # if there is token '(' call read_tail recursively
        exp = read_tail(src)
        return exp
        # END PROBLEM 1
    elif val == "'":
        ...
        # END PROBLEM 7B
    # val not in DELIM which is operator or number
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # remove dot
            src.remove_front()
            arg = src.remove_front()

            # well-formed list
            if arg == '(':
                exp = read_tail(src)
                if src.current() not in DELIMITERS:
