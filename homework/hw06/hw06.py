...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    input_pass_list = []
    def transaction_withdraw(amount, input_pass):
        nonlocal balance
        if len(input_pass_list) >= 3:
            return 'Your account is locked. Attempts: ' +  "['"+ input_pass_list[0] + "', '" + input_pass_list[1] + "', '" + input_pass_list[2] + "']"
        elif not input_pass == password:
            input_pass_list.append(input_pass)
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    # you can access your account either through your old or new passwd
    transaction_output = withdraw(0, old_password)
    if type(transaction_output) == str:
        return transaction_output
    else:
        return lambda amount, passwd: withdraw(amount, old_password) if passwd == old_password or passwd == new_password else withdraw(amount, passwd)
...
class MissManners:
    ...
    def ask(self, *args):
        # Check if it includes politeness
        operator = args[0]
        operand = args[1:]
        if 'please' in str(operator):
            # Check arributes of objects
            operator = operator[7:]
            if hasattr(self.obj, operator):
                # unpacked arguments
                return getattr(self.obj, operator)(*operand)
            # No match
            else:
                return 'Thanks for asking, but I know not how to ' + str(operator) + '.'
        else:
